...
const ADMIN_EMAIL    = process.env.ADMIN_EMAIL || 'certificates@aarhiimpactfoundation.org';
...
async function notifyAdmin(record, ip) {
  ...
  try {
    ...
    await fetch('https://api.resend.com/emails',{
      ...
      body:JSON.stringify({
        from:'Aarhi Impact Foundation <certificates@aarhiimpactfoundation.org>',
        to:[ADMIN_EMAIL],
...
export default async function handler(req,res) {
  ...
  try {
    ...
    if (record.status==='revoked')
      return res.status(410).json({error:'This certificate has been revoked. Contact certificates@aarhiimpactfoundation.org for assistance.'});

    // Resend action — trigger GitHub Action
    if (action==='resend') {
      if (!GH_PAT||!GH_DISPATCH_URL)
        return res.status(500).json({error:'Resend not configured. Contact certificates@aarhiimpactfoundation.org'});
      try {
        ...
        return res.status(500).json({error:'Resend failed. Please contact certificates@aarhiimpactfoundation.org'});
      } catch(e) {
        return res.status(500).json({error:'Resend failed. Please contact certificates@aarhiimpactfoundation.org'});
      }
    }
